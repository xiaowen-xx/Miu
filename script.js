...
function getFullPersona(chat) {
    ...
    const timeStr = `${now.getFullYear()}年${now.getMonth() + 1}月${now.getDate()}日 ${weekMap[now.getDay()]} ${String(now.getHours()).padStart(2, '0')}:${String(now.getMinutes()).padStart(2, '0')}`;
...
async function generateAiReply(chat, isRegenerate = false) {
    ...
    // 1. 找到上一条【有效】消息（排除正在loading的那条）
    const validHistory = chat.messages.filter(m => !m.isLoading);
    const lastMsg = validHistory[validHistory.length - 1];
    
    let timeGapPrompt = "";
    
    if (lastMsg && lastMsg.timestamp) {
        const diffMs = now.getTime() - lastMsg.timestamp; // 计算毫秒差
        const diffMins = Math.floor(diffMs / (1000 * 60)); // 转为分钟

        // 如果时间差超过 20 分钟，强制注入时间感知指令
        if (diffMins > 20) {
            let durationDesc = "";
            if (diffMins < 60) durationDesc = `${diffMins}分钟`;
            else if (diffMins < 1440) durationDesc = `${(diffMins/60).toFixed(1)}小时`;
            else durationDesc = `${(diffMins/1440).toFixed(1)}天`;

            timeGapPrompt = `
【强效时间感知系统】：
检测到【对话中断】！
距离上一条消息已经过去了：${durationDesc}。
上一条消息的时间是：${lastMsg.time}。
而现在的真实时间是：${timeStr}。
【指令】：
1. 你的体感时间必须与现实同步。如果从下午到了晚上，请表现出“到了晚上”的语境。
2. 如果间隔时间很久（超过数小时），严禁像刚才还在聊天一样秒回！你需要表现出“好久不见”或“刚才去忙了/刚才没看手机”的真实人类反应。
`;
        }
    }
    ...
    // ★★★ 注意：这里把 timeGapPrompt 加到了 systemPrompt 的最后面，确保权重最高 ★★★
...
function minimizeVoiceCallToFloat() {
    ...
    // 设置悬浮窗头像
    // 优先用 userAvatar (这其实是对方头像变量名设计遗留问题，vcCharAvatar 用的就是 chat.avatar)
    // 简单点：直接取当前大屏上的头像 src
    ...
    // ★ 关键：这里不要清除计时器 (clearInterval)，让它在后台继续跑
    // 也不要设置 isVcConnecting = false
}
