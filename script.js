...
function renderMessages(chat) {
    ...
    msgsToRender.forEach((msg, relativeIndex) => { 
        ...
        if (isSelf) {
            ...
            row.innerHTML = `${checkboxHtml}<div class="bubble-wrapper" style="justify-content: flex-end;">${timeHtml}${stackHtml}</div>${avatarHtml}`;
        } else {
            let avatarHtml = showAi ? `<img src="${otherAvatar}" class="avatar-img" style="${shouldRenderAvatar ? '' : 'visibility:hidden;'}">` : '';
            row.innerHTML = `${avatarHtml}<div class="bubble-wrapper" style="justify-content: flex-start;">${stackHtml}${timeHtml}</div>${checkboxHtml}`;
        }
...
function openStickerView() {
    ...
    document.getElementById('stickerSubView').classList.add('active');
    
    // ★ 新增：给面板加上 sticker-mode 类 ★
    document.getElementById('chatToolsPanel').classList.add('sticker-mode');
}

function backToToolsMenu() {
    document.getElementById('stickerSubView').classList.remove('active');
    document.getElementById('toolsMainMenu').style.display = 'flex';
    
    // ★ 新增：移除 sticker-mode 类 ★
    document.getElementById('chatToolsPanel').classList.remove('sticker-mode');
}
...
function simulateCharacterLife() {
    ...
    chatList.forEach(chat => {
        ...
        // 3. 概率触发：并不是冷却结束就马上发，而是每分钟有一定概率触发
        // 设定为 1/120 的概率，平均冷却结束后 2 小时左右会发一条
        // 你可以调大这个数字让它更容易触发 (比如 0.05)
...
【任务目标】
根据当前模式、时间、世界观，决定执行以下**其中一项**操作：

1. **POST (发动态)**: 
   - 必须符合你的【世界观】（例如：如果是古代背景，不要提手机；如果是末世，描述环境要符合末世）。
   - 生活感强，口语化。

2. **COMMENT (评论)**: 
   - 评论用户的最新动态（如果有关联的话）。
   - 必须像真人刷朋友圈一样简短！严禁写成书信体，严禁长篇大论，严禁过于客气。
3. **NONE**: 
   - 此时此刻不想操作。
    ...
    try {
        const response = await fetch(`${endpoint}/chat/completions`, {
            ...
            body: JSON.stringify({
                model: model,
                messages: [{ role: "user", content: systemPrompt }],
                temperature: 0.85 // 略微提高创造性
            })
        });

        // ... (后续的 JSON 解析和保存逻辑保持不变，不需要修改) ...
        const data = await response.json();
